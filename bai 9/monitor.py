...
class TrafficMonitor:
    def __init__(self):
        ...
        self.max_plausible_kmh = 180   # ngưỡng lọc: tốc độ vượt mức này -> nghi SOURCE sai, không tin
    ...
    def speeds(self, detections):
        ...
        for tid, cid, (_, y) in zip(
            ...
            # ⚠️ Sanity check: tốc độ phi thực tế -> SOURCE chưa hiệu chỉnh đúng, không tin số này
            if speed > self.max_plausible_kmh:
                if not self._warned_calibration:
                    print(f"[CANH BAO] Toc do {int(speed)}km/h phi thuc te -> "
                          f"PERSPECTIVE_SOURCE trong config.py chua duoc hieu chinh dung. "
                          f"Xem lai muc 'Hieu chinh PERSPECTIVE_SOURCE'.")
                    self._warned_calibration = True
                labels.append(f"#{tid} {name} ?km/h")
                continue
